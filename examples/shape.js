...
class Wireframe {
  constructor(ctx, maxVertices = 10000) {
    _defineProperty(this, "_bigBuffer", void 0);

    _defineProperty(this, "_ctx", void 0);

    _defineProperty(this, "_shader", void 0);

    _defineProperty(this, "_maxVertices", 0);

    _defineProperty(this, "_numberVertices", 0);

    _defineProperty(this, "_vertices", void 0);

    _defineProperty(this, "_vIdx", 0);

    _defineProperty(this, "_color", void 0);

    _defineProperty(this, "_mvp", void 0);
  ...
  drawLine(x, y, x2, y2, color = this._color) {
    this._check(2);
  ...
  drawArrow(x, y, x2, y2, color = this._color) {
    this._check(6);
  ...
  drawRectline(x, y, x2, y2, width, color = this._color) {
    this._check(8);
  ...
  drawTriangle(x, y, x2, y2, x3, y3, color = this._color, color2 = this._color, color3 = this._color) {
    this._check(6);
  ...
  drawQuad(x, y, x2, y2, x3, y3, x4, y4, color = this._color, color2 = this._color, color3 = this._color, color4 = this._color) {
    this._check(8);
  ...
  drawRect(x, y, width, height, color = this._color) {
    // Matrix2D.SRT()
    let r = x + width;
    let d = y + height;
  ...
  drawX(x, y, size, color = this._color) {
    let l = x - size,
        r = x + size,
        t = y - size,
        b = y + size;
  ...
  drawCircle(x, y, radius, segments = 0, color = this._color) {
    this.drawEllipse(x, y, radius, radius, segments, color); // if (segments === 0)
  ...
  drawEllipse(x, y, width, height, segments = 10, color = this._color) {
  ...
  drawCurve(x1, y1, cx1, cy1, cx2, cy2, x2, y2, segments, color = this._color) {
    this._check(segments * 2 + 2); // Algorithm from: http://www.antigrain.com/research/bezier_interpolation/index.html#PAGE_BEZIER_INTERPOLATION
  ...
  _check(numVertices) {
    if (numVertices + this._numberVertices > this._maxVertices) this.flush();
  }
  ...
  _vertex(x, y, color) {
    ...
    vertices[this._vIdx++] = color.a; // u v
...
if (gl) {
  ...
  renderer.penColor = Color.YELLOW; // let m = Matrix2D.SRT(1, 1, 0 * Math.PI / 180, 0, 0);

  let proj = center2D(gl.drawingBufferWidth, gl.drawingBufferHeight); // m.append(proj);
  ...
  renderer.penColor = Color.random(); // renderer.drawLine(0, 200, 500, 200);
  ...
  renderer.drawCurve(-300, 0, -100, -300, 100, -600, 300, 0, 50);
  renderer.flush();
} else {
